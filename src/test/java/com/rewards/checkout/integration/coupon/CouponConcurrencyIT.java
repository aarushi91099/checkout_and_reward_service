...
class CouponConcurrencyIT extends AbstractIntegrationTest {

    @Test
    void exactlyOneOfTwoConcurrentCheckoutsRedeemingTheSameCouponSucceeds() throws Exception {
        UUID productId = seedProduct("Widget", "20.00", 10);
        seedCoupon("SHARE10", "10", 9001L);
        ...
        assertThat(couponRepository.findById("SHARE10").orElseThrow().getStatus()).isEqualTo(CouponStatus.REDEEMED);
        assertThat(jdbcTemplate.queryForObject(
                        "select count(*) from orders where coupon_code = 'SHARE10'", Integer.class))
                .isEqualTo(1);
    }
    ...
    private ResponseEntity<String> race(UUID cartId, CountDownLatch ready, CountDownLatch start) throws InterruptedException {
        ...
        return checkout(cartId, "key-" + cartId, "SHARE10");
    }
}
